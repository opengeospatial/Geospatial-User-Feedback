...
The User Feedback Item requirements class defines the data model classes that are involved in the definition of an individual _User Feedback item_. A _User Feedback Item_ is the container of the actual feedback. Every item is set into a context by a  _target_.

The User Feedback Item requirements Class defines the requirements for a _User Feedback Item_, which is the atomic unit of information of a _User Feedback Collection_.
...
<<tbl_feedback_item>> define the set of properties that may be included in a User Feedback Item.
...
The User Feedback Collection requirements class is a specific implementation of the record collection defined in the https://docs.ogc.org/is/20-004r1/20-004r1.html#clause-record-collection[OGC API - Records Standard] (Section 7.3) where the conformsTo property takes the value "http://www.opengis.net/spec/ogcapi-feedback-2/2.0/conf/feedback" to indicate that all elements in the collection follow the <<feedback_item, User Feedback Item>> requirements.
